...
def pull_ogsl():
    ...
    # Pull ogsl from Oracc on GitHub
    response = requests.get('https://raw.githubusercontent.com/oracc/ogsl/master/00lib/ogsl.asl')
    ogsl = response.text.splitlines()

    # If the url doesn't work, here is the backup as of 9/23/2019
    #path = os.path.dirname(os.path.dirname(__file__)) + '/data/ogsl.txt'
    #with open(path) as file: ogsl = file.readlines()

    possible_keys = list()

    for line in ogsl:
        key = line[0:line.expandtabs().find(' ')].strip()
        value = line[line.expandtabs().find(' '):].strip()
        if key != '': possible_keys.append(key.strip())

    for k in set(possible_keys):
        print(k, '\t', possible_keys.count(k))
